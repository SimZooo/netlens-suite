...
impl FromStr for ScanType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        match s {
            ...
            _ => {
                Err(String::from("Invalid scan type"))
            }
        }
    }
}
...
struct Args {
    ...
    #[arg(short, long, value_parser = value_parser!(ScanType))]
    scan: Option<ScanType>,
    #[arg(short, long)]
    ports: Option<String>,
    #[arg(short = 'g', long)]
...
fn create_output(replies: &Vec<u16>, all_ports: Vec<u16>, scan: ScanType, dest_ip: Ipv4Addr) -> ColoredString {
    ...
    if scan == ScanType::ACK {
        ...
        for reply in replies.iter() {
            ...
            let service = banner.lines().next().unwrap_or("N/A");
            output_text = format!(
                "{}| {:>4}/tcp    {}\n",
                output_text,
                reply, 
...
fn create_json_output(replies: &Vec<u16>, all_ports: Vec<u16>, out_json: bool, scan: ScanType, dest_ip: Ipv4Addr, src_port: u16, src_ip: Ipv4Addr) -> Vec<Service> {
    ...
    if scan == ScanType::ACK {
        ...
        for reply in replies.iter() {
            ...
            let service = banner.lines().next().unwrap_or("N/A");
            let re = Regex::new(r"([A-Za-z\-]+)[_\/ ]?([0-9]+(\.[0-9]+)+)").unwrap();
            let capture = re.captures(service);
            if let Some(capture) = capture {
                let service_name = capture.get(0).unwrap().as_str();
                println!("{:?}", capture);
                let version = capture.get(2).unwrap().as_str();
...
fn main() {
    ...
    let mut output_text = ColoredString::from("");
    if !args.json {
        output_text = create_output(&replies, all_ports, scan, dest_ip);
    } else if args.json {
        let services = create_json_output(&replies, all_ports, args.json, scan, dest_ip, src_port, src_ip);
        output_text = ColoredString::from(json!(services).to_string());
    }
